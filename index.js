...
function entropy (trait_entropy_list) {
    return trait_entropy_list.reduce((initialValue, currentValue) => 
        initialValue - currentValue * Math.log2(currentValue), 0)
}
  
function informationContent (trait_info_list) {
    return trait_info_list.reduce((initialValue, currentValue) => 
        initialValue - Math.log2(currentValue), 0)
}

function calculateRarity(meta_data, ranker = true, blank_trait = '__undefined') {
    let trait_map = {}, trait_map_stat = {};
    let trait_info_list = [], trait_entropy_list = [];
    ...
    meta_data.forEach((NFT) => {
        trait_list.except(Object.keys(NFT))
        .forEach((missing_trait) => {
